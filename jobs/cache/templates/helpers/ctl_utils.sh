...
prep_system() {
  # bump maximum connections for higher TCP backlog setting
  echo 512 > /proc/sys/net/core/somaxconn || true

  # enable memory overcommit to quell warnings in redis startup log
  sysctl vm.overcommit_memory=1 || true

  # disable Transparent Huge Pages (THP) for redis' malloc discipline
